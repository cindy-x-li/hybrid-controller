% =========================================================================
% RECEDING HORIZON SUPERVISOR
% includes perturbation test 
% switches during docking, no controller locking mechanism
% =========================================================================
clear; clc; close all;
...
fprintf('Running Hardware-Safe Predictive Supervisor...\n');
...
for k = 1:length(time_steps)-1
    ...
    % --- STEP 3: ASYMMETRIC SELECTION LOGIC ---
    % predicted future overshoot
    if ~is_fast_safe
        % DANGER: switch to Slow and start the hardware lock timer
        Kp_active = Kps;
        Ki_active = Kis;
        active_controller(k) = 2;
        slow_lock_timer = lockout_duration; % Reset the cooldown!
        
    elseif slow_lock_timer <= 0
        % SAFE & UNLOCKED: The cooldown is over, we can safely use Fast
        Kp_active = Kpf;
        Ki_active = Kif;
        active_controller(k) = 1;
        
    else
        % SAFE BUT LOCKED: no switching to fast to protect pump from chatter
        Kp_active = Kps;
        Ki_active = Kis;
        active_controller(k) = 2;
    end
...
title('Hybrid Water Level (ode45 Prediction)'); ylabel('Height (cm)'); grid on;
